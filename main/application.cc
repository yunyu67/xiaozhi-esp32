...
void Application::OnWakeWordDetected()
{
    ...
    if (music && music->IsDndModeEnabled() && music->IsDownloading())
    {
        ...
        if (codec) {
            codec->SetOutputVolume(codec->GetOutputVolume() / 3);  // 降低音量到1/3
        }
        return;
    }
...
void Application::SetDeviceState(DeviceState state)
{
    ...
    // 但如果音乐播放器处于勿扰模式(DND)，则不停止音乐
    if (previous_state == kDeviceStateIdle && state != kDeviceStateIdle)
    {
        auto music = board.GetMusic();
        if (music)
        {
            // 检查是否启用了勿扰模式 - 如果是音乐播放中，则保持播放
            if (music->IsDndModeEnabled() && music->IsDownloading())
            {
                ESP_LOGI(TAG, "Music DND mode active - keeping music playing during state change: %s -> %s",
    ...
    // 当回到Idle状态时，检查是否有待处理的唤醒词
    if (state == kDeviceStateIdle)
    {
        auto music = board.GetMusic();
        if (music && music->HasPendingWakeWord())
        {
            ESP_LOGI(TAG, "Processing pending wake word after music stopped");
